...
public class Console
{
    ...
    * 
    * @param options
    *           a string describing the options, with each option enclosed in []
   ...
   {
      String input = menu("[P]lain text  [O]ne time text");
      String recipientName = readInput("To");
      String body = readText("Message text");
      Text aMessage = new PlainText("");
      if (input.equals("P"))
      {
         aMessage = new PlainText(body);

      } else if (input.equals("O"))
      {
         aMessage = new OneTimeText(body);
      }

      if (system.sendMessage(new Message(sender.getUsername(), aMessage), recipientName))
    ...
    * 
    * @param user
    *           the user who is reading messages
    */
